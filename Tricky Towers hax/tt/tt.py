...
def main():
    ...
    iu = InputUtil()
    iu.set_active_window('Tricky Towers')
    iu.bind_hotkey(keys=VK['NEXT'],     timeout=0,    func=exit_app)
    iu.bind_hotkey(keys=VK['END'],      timeout=1,    func=quick_restart)
    iu.bind_hotkey(keys=VK['RCONTROL'],               func=press,         args=['a'])
    iu.bind_hotkey(keys=VK['NUMPAD0'],                func=press,         args=['d'])
    iu.bind_hotkey(keys=VK['UP'],       timeout=0.08, func=press,         args=['w'])
    iu.bind_hotkey(keys=VK['KEY_E'],    timeout=0.08, func=press,         args=['w'])
    iu.start()
